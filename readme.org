...
** Motivacion
   ...
   aproximacion con mas de *C* que de *Go*, carente de un diseño orientado a
   objetos, con chapuzas varias... pero bueno, el repositorio debia tener un
   inicio, no?
...
** Sintaxis
   ...
   - busqueda de texto en cualquier ubicacion:
     ...
     #+BEGIN_SRC go
       regexp3.Regexp3( "Raptor Test", "Raptor" );
     #+END_SRC

   - multiples opciones de busqueda "exp1|exp2"

     #+BEGIN_SRC go
       regexp3.Regexp3( "Raptor Test", "Dinosaur|T Rex|Raptor|Triceratops" );
     #+END_SRC

   - coincidencia con cualquier caracter '.'

     #+BEGIN_SRC go
       regexp3.Regexp3( "Raptor Test", "R.ptor" );
     #+END_SRC

   - coincidencia cero o una ves '?'

     #+BEGIN_SRC go
       regexp3.Regexp3( "Raptor Test", "Ra?ptor" );
     #+END_SRC

   - coincidencia una o mas veces '+'

     #+BEGIN_SRC go
       regexp3.Regexp3( "Raaaptor Test", "Ra+ptor" );
     #+END_SRC
   ...
   - rango de coincidencias "{n1,n2}"
     ...
     #+BEGIN_SRC go
       regexp3.Regexp3( "Raaaptor Test", "Ra{0,100}ptor" );
     #+END_SRC

   - numero de coincidencias especifico '{n1}'

     #+BEGIN_SRC go
       regexp3.Regexp3( "Raptor Test", "Ra{1}ptor" );
     #+END_SRC

   - numero minimo de coincidencias '{n1,}'

     #+BEGIN_SRC go
       regexp3.Regexp3( "Raaaptor Test", "Ra{1,}ptor" );
     #+END_SRC

   - Rango de caracteres "x-n"

     #+BEGIN_SRC go
       regexp3.Regexp3( "Raptor Test", "Ra-zptor" );
     #+END_SRC
   ...
   - backreferences "@id"
     ...
     #+BEGIN_SRC go
       regexp3.Regexp3( "ae_ea", "<a><e>_@2@1" )
     #+END_SRC
   ...
   - *negacion de exprecion* "([^(exp)])" o "<[^(exp)]>"
     ...
     #+BEGIN_SRC go
       regexp3.Regexp3( "a123456789b", "a([^(b)])*b" );
     #+END_SRC
   ...
   - modificadores de comportamiento
     ...
     - busqueda solo al final '#$exp'
       ...
       #+BEGIN_SRC go
         regexp3.Regexp3( "Raptor Test", "#$Test" );
       #+END_SRC

     - busqueda al inicio y final "#^$exp"

       #+BEGIN_SRC go
         regexp3.Regexp3( "Raptor Test", "#^$Raptor Test" );
       #+END_SRC
     ...
     - ignorar entre minusculas y mayusculas "exp#*"
       ...
       #+BEGIN_SRC go
         regexp3.Regexp3( "Raptor Test", "(RaPtOr)#* TeS#*t" );
       #+END_SRC
...
** Capturas
   ...
   indice incorrecto regresara una cadena vacia.
...
*** Colocar capturas dentro de una cadena
    ...
    de una captura coloque el signo '#' seguido del indice de captura. por ejemplo
    el argumento =putStr= podria ser

    #+BEGIN_SRC go
      pText := "captura 1 >>#1<< captura 2 >>#2<< captura 747 >>#747<<"
    #+END_SRC
...
** algunos ejemplos de uso
   ...
   #+BEGIN_SRC go
     regexp3.Regexp3( "07-07-1777", "<0?[1-9]|[12][0-9]|3[01]><[/:-\\]><0?[1-9]|1[012]>@2<[12][0-9]{3}>" );
   #+END_SRC
   ...
   #+BEGIN_SRC go
      regexp3.Regexp3( "https://en.wikipedia.org/wiki/Regular_expression", "(https?|ftp):://<[^:s/:<:>]+></[^:s:.:<:>,/]+>*<.>*" );
   #+END_SRC

   capturar algo parecido a un enlace web

   #+BEGIN_SRC go
     regexp3.Regexp3( "<mail>nasciiboy@gmail.com</mail>", "<[_A-Za-z0-9:-]+(:.[_A-Za-z0-9:-]+)*>:@<[A-Za-z0-9]+>:.<[A-Za-z0-9]+><:.[A-Za-z0-9]{2}>*" );
   #+END_SRC

   capturar por secciones (usuario,sitio,dominio) algo parecido a un correo.

   #+BEGIN_SRC go
      regexp3.Regexp3( "aa0123aa", "<aa><[^(aa)]>*<aa>" );
   #+END_SRC
...
* Todo
  ...
  3. Hacer que la libreria tenga un enfoque orientado a objetos:

     Cada instancia sera independiente una de otra, contendra su propia regexp,
     cadena de busqueda y capturas

  4. Soporte de caracteres utf-8

  5. Incluir la "negacion" de constructores

  6. Pruebas de rendimiento

  7. Hacer un poco mas "funcional" algunas funciones

  8. Paralelizar la busqueda de rutas
